...
static class World
{
    ...
    static bool fighting_currently;
    
    public static void Fighting(Player player, Monster monster)
    {
        fighting_currently = true;

        while (fighting_currently)
        {
            ...
            switch (Console.ReadLine())
            {
                case "1":
                    break;
                case "2": fighting_currently = false;
                    break;
                default: fighting_currently = false;
                    break;
            }
        }
    ...
    static void Encounter(Player player, Monster monster)
    {
        ...
        while (true)
        {
            int dealt = RandomGenerator.Next(player.CurrentWeapon.MinimumDamage, player.CurrentWeapon.MaximumDamage) + player.CurrentHitPoints;
            if (player.MaximumHitPoints < dealt)
            {
                dealt = player.MaximumHitPoints;
            }

            monster.Healthpoints -= dealt;
            ...
            Console.WriteLine($"You deal {dealt} damage to the {monster.Name}, they have {monster.Healthpoints} Hp left");

            int received = monster.CurrentHitPoints;
            player.Healthpoints -= received;
            ...
            Console.WriteLine($"The {monster.Name} deals {received} damage, you have {player.Healthpoints} Hp left ");
